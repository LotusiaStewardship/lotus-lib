...
async function main() {
  ...
  alice.on(ConnectionEvent.CONNECTED, (peer: PeerInfo) => {
    console.log(`Alice: Connected to ${peer.peerId}`)
  })

  alice.on(ConnectionEvent.MESSAGE, (message, from: PeerInfo) => {
    console.log(`Alice received from ${from.peerId}:`, message.payload)
  })

  bob.on(ConnectionEvent.CONNECTED, (peer: PeerInfo) => {
    console.log(`Bob: Connected to ${peer.peerId}`)
  })

  bob.on(ConnectionEvent.MESSAGE, (message, from: PeerInfo) => {
    console.log(`Bob received from ${from.peerId}:`, message.payload)
  })
